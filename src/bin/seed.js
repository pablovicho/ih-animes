...
const Anime = require('./../models/anime') //la ruta relativa es con los archivos que creamos, el nombre solito con los que tiene node

require('dotenv').config() //¿por qué? en index.js decíamos que íbamos a usar dotenv arriba, y abajo hacíamos el connectDB, pero seed no se va a conectar con ningún otro archivo, así que require todas las importaciones
...
// CONNECT DB, conectar con la base de datos
mongoose.connect(process.env.MONGODB) // no es port, es la base de datos en la nube, mongoDB

// POPULATE DB, poblar la base de datos
const generateAnimes = async() =>{
    await Anime.create(animes) //aguanta hasta que el anime cree todos los animes. create es un método de mongoose
console.log('Base de datos poblada con animes japos kawaii')
mongoose.connection.close() //cerrar la conexión, para no tenerla todo el tiempo abierta. un método de mongoose
}
